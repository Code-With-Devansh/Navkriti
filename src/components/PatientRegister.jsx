...
import { Monitor, Users, AlertCircle, Calendar, Plus, User, Phone, Lock, MapPin, Stethoscope, UserPlus, Pill, Trash2, X } from 'lucide-react';
...
export default function PatientRegister() {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name || !formData.ph_number || !formData.age || !formData.password) {
    ...
    try {
      const token = localStorage.getItem('adminToken') || localStorage.getItem('patientToken');
      ...
      if (!patientData.success) {
        setMessage({ type: 'error', text: patientData.error || 'Failed to create patient' });
        setLoading(false);
        return;
      }
      ...
      if (validMedicines.length > 0 && formData.dept) {
        const medicineResponse = await fetch(`/api/admin/patients/${patientId}/add-medicine`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
        ...
        <div className="patient-register-container">
          ...
          {message.text && (
            <div className={`mb-6 p-4 rounded-lg flex items-start gap-3 ${
              ...
              <div>
                <p className="font-medium">{message.type === 'success' ? 'Success' : message.type === 'warning' ? 'Warning' : 'Error'}</p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleMedicineChange(index, 'color', e.target.value)}
                        className="w-full px-4 py-3 border-b-2 border-gray-200 focus:border-blue-500 outline-none transition bg-white"
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleMedicineChange(index, 'acceptedtime', e.target.value)}
                        className="w-full px-4 py-3 border-b-2 border-gray-200 focus:border-blue-500 outline-none transition"
                        placeholder="8-10, 14-16, 20-22"
                      />
                      <p className="text-xs text-gray-500 mt-1">Format: 8-10, 14-16, 20-22</p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleMedicineChange(index, 'duration_days', parseInt(e.target.value))}
                        className="w-full px-4 py-3 border-b-2 border-gray-200 focus:border-blue-500 outline-none transition"
                        placeholder="7"
                      />
                    </div>
